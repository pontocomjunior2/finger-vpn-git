...
class DatabaseErrorAnalyzer:
    ...
    def get_recovery_action(
        ...
        base_action = self.recovery_strategies.get(
            category, self.recovery_strategies[ErrorCategory.UNKNOWN]
        )
...
class DatabaseErrorHandler:
    ...
    async def handle_error_async(
        self, error: Exception, context: ErrorContext, operation: Callable
    ) -> Any:
        """Handle database error asynchronously"""
        return await self._handle_error_internal(
            error, context, operation, is_async=True
        )
        ...
        try:
            if recovery_action.strategy == RecoveryStrategy.RETRY:
                return await self._execute_retry(
                    error, context, operation, recovery_action, is_async
                )

            elif recovery_action.strategy == RecoveryStrategy.RECONNECT:
                return await self._execute_reconnect(
                    error, context, operation, recovery_action, is_async
                )
                ...
                return await self._execute_escalation(
                    error, context, operation, recovery_action, is_async
                )

            else:  # ABORT
                return await self._execute_abort(
                    error, context, operation, recovery_action, is_async
                )
    ...
    async def _execute_retry(
        ...
        if context.attempt_number >= action.max_attempts:
            logger.error(
                f"Max retry attempts ({action.max_attempts}) exceeded for {context.operation}"
            )
            raise error

        logger.info(
            f"Retrying {context.operation} in {action.delay:.2f}s (attempt {context.attempt_number + 1})"
        )
    ...
    async def _execute_abort(
        ...
        logger.error(
            f"Aborting operation {context.operation} due to unrecoverable error: {error}"
        )
        raise error
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                return await handler.handle_error_async(
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                handler = get_database_error_handler()
                return handler.handle_error_sync(
                    e, context, lambda: func(*args, **kwargs)
                )
