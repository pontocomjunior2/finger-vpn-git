    ...
    def __post_init__(self):
        if not self.metrics:
            self.metrics = TransactionMetrics(
                transaction_id=self.transaction_id, start_time=self.start_time
            )
...
class TransactionMonitor:
            ...
            for tx_id, tx_info in list(self.active_transactions.items()):
                ...
                # Check for excessive queries
                    ...
                    problematic_transactions.append(
                        (tx_id, "excessive_queries", len(tx_info.queries_executed))
                    )
    ...
    def update_transaction_activity(
        self,
        transaction_id: str,
    ...
    def add_savepoint(self, transaction_id: str, savepoint_name: str):
        ...
        with self._lock:
            if transaction_id in self.active_transactions:
                ...
                logger.debug(
                    f"Savepoint {savepoint_name} added to transaction {transaction_id}"
                )
    ...
    def complete_transaction(
        ...
        with self._lock:
            if transaction_id in self.active_transactions:
                ...
                tx_info.metrics.duration = (
                    tx_info.metrics.end_time - tx_info.metrics.start_time
                )
                ...
                if self._performance_samples:
                    self._stats["avg_duration"] = sum(self._performance_samples) / len(
        ...
        with self._lock:
            ...
            return {
                ...
                "recent_stats": {
                    ...
                    "success_rate": (
                        recent_successful / max(len(recent_transactions), 1)
                    )
                    * 100,
                },
                "deadlock_stats": {
...
class TransactionManager:
        ...
        async with self.db_manager.get_connection(context, timeout) as conn:
            ...
            try:
                ...
                tx_context = TransactionContext(
                    connection=conn, transaction_info=tx_info, monitor=self.monitor
                )
                ...
                try:
                    ...
                    self.monitor.complete_transaction(
                        transaction_id, TransactionStatus.FAILED, rollback_error
                    )

                raise

    @contextmanager
    def managed_transaction_sync(
        self,
        context: str = "unknown",
        timeout: Optional[float] = None,
        priority: TransactionPriority = TransactionPriority.NORMAL,
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if tx_info.isolation_level != "READ_COMMITTED":
                    cursor.execute(
                        f"SET TRANSACTION ISOLATION LEVEL {tx_info.isolation_level}"
                    )
            ...
            logger.warning(
                f"Failed to configure transaction {tx_info.transaction_id}: {e}"
            )
            conn.rollback()
    ...
    def _is_timeout_error(self, error: Exception) -> bool:
        ...
        return any(
            indicator in error_str
            for indicator in ["timeout", "statement timeout", "lock timeout"]
        )
        ...
        for tx_info in self.monitor.get_active_transactions():
            ...
            if duration > threshold_seconds:
                if self.monitor.force_rollback_transaction(
                    tx_info.transaction_id, f"exceeded_threshold_{threshold_seconds}s"
                ):
                    rolled_back.append(tx_info.transaction_id)

        if rolled_back:
            logger.warning(
                f"Force rolled back {len(rolled_back)} long-running transactions"
            )
...
class TransactionContext:
    ...
    def __init__(
        self, connection, transaction_info: TransactionInfo, monitor: TransactionMonitor
    ):
    ...
    def execute_query(self, query: str, parameters: tuple = None, fetch: bool = False):
        ...
        try:
            ...
            logger.error(
                f"Query failed in transaction {self.transaction_info.transaction_id}: {e}"
            )
            raise
    ...
    def rollback_to_savepoint(self, savepoint_name: str):
        ...
        logger.debug(
            f"Rolled back to savepoint {savepoint_name} in transaction {self.transaction_info.transaction_id}"
        )
