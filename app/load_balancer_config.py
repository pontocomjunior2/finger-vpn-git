...
def create_load_balance_config() -> LoadBalanceConfig:
    ...
    # Safety limits
    config.min_instances_for_rebalance = int(
        os.getenv("MIN_INSTANCES_FOR_REBALANCE", "2")
    )
...
def get_rebalance_intervals() -> dict:
    ...
    return {
        "smart_rebalance_interval": int(
            os.getenv("SMART_REBALANCE_INTERVAL", "300")
        ),  # 5 minutes
        "consistency_check_interval": int(
            os.getenv("CONSISTENCY_CHECK_INTERVAL", "180")
        ),  # 3 minutes
        "performance_monitor_interval": int(
            os.getenv("PERFORMANCE_MONITOR_INTERVAL", "60")
        ),  # 1 minute
        "min_rebalance_interval": int(
            os.getenv("MIN_REBALANCE_INTERVAL", "60")
        ),  # 1 minute minimum
    }
...
def validate_config(config: LoadBalanceConfig) -> bool:
    ...
    if (
        config.emergency_threshold <= config.max_load_factor
        or config.emergency_threshold > 1.0
    ):
        return False
