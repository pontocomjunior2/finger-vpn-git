...
class MonitoringSetup:
    ...
    async def setup_dashboard_and_notifications(self):
        ...
        self.dashboard_components = setup_comprehensive_monitoring(
    ...
    def validate_setup(self):
        ...
        if self.monitoring_system:
            validation_results.append(
                (
                    "Monitoring System",
                    {"valid": True, "message": "Created successfully"},
                )
            )
        else:
            validation_results.append(
                ("Monitoring System", {"valid": False, "message": "Not created"})
            )

        # Check if API is setup
        if self.monitoring_api:
            validation_results.append(
                ("Monitoring API", {"valid": True, "message": "Setup successfully"})
            )
        else:
            validation_results.append(
                ("Monitoring API", {"valid": False, "message": "Not setup"})
            )
        ...
        for component, result in validation_results:
            ...
            logger.info(
                f"  {status} {component}: {result.get('message', 'Unknown status')}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--action",
        choices=["setup", "run", "validate", "config"],
        default="setup",
    ...
    try:
        if args.action == "setup":
            ...
            logger.info(
                "Monitoring system setup complete. Use --action=run to start server."
            )
...
def setup_orchestrator_with_monitoring(orchestrator_class, db_config=None):
    ...
    async def _setup():
        ...
        # Add monitoring to orchestrator class
        monitored_orchestrator_class = setup.add_orchestrator_monitoring(
            orchestrator_class
        )
...
def example_orchestrator_integration():
    ...
    # Add monitoring
    MonitoredOrchestrator, monitoring_system = setup_orchestrator_with_monitoring(
        ExampleOrchestrator
    )
