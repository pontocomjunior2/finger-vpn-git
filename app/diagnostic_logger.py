...
class StructuredLogger:
    ...
    def warning(
        ...
        self._log_with_context(
            "WARNING", message, context, suggested_solution=suggested_solution
        )
    ...
    def critical(
        ...
        self._log_with_context(
            "CRITICAL", message, context, exception, suggested_solution
        )
...
class EnvironmentValidator:
    ...
    def validate_all(self) -> Dict[str, Any]:
        ...
        if total_issues > 0:
            ...
            self.logger.error(
                f"Environment validation failed with {total_issues} issues"
            )
        elif total_warnings > 0:
            validation_report["overall_status"] = "WARNING"
            self.logger.warning(
                f"Environment validation completed with {total_warnings} warnings"
            )
        else:
    ...
    def _validate_database_config(self) -> Dict[str, Any]:
            ...
            if not value:
                if var_name == "DB_PASSWORD":
                    result["warnings"].append(
                        f"{var_name} is not set - using empty password"
                    )
                    result["recommendations"].append(
                        f"Set {var_name} for secure database access"
                    )
                else:
                ...
                try:
                    ...
                    result["issues"].append(
                        f"{var_name} is not a valid integer: {value}"
                    )
        ...
        try:
            ...
            if not (1 <= port_num <= 65535):
                result["issues"].append(
                    f"DB_PORT {port_num} is outside valid range (1-65535)"
                )
        except ValueError:
    ...
    def _validate_redis_config(self) -> Dict[str, Any]:
        ...
        for var_name, var_info in redis_vars.items():
            ...
            if not value and not var_info.get("optional"):
                result["warnings"].append(
                    f"{var_name} is not set - using default: {var_info['default']}"
                )
        ...
        try:
            ...
            if not (1 <= port_num <= 65535):
                result["issues"].append(
                    f"REDIS_PORT {port_num} is outside valid range (1-65535)"
                )
        except ValueError:
            pass
    ...
    def _validate_orchestrator_config(self) -> Dict[str, Any]:
        ...
        orchestrator_vars = {
            "MAX_STREAMS_PER_INSTANCE": {
                "default": "20",
                "description": "Maximum streams per instance",
                "type": "int",
            },
            "HEARTBEAT_TIMEOUT": {
                "default": "300",
        ...
        for var_name, var_info in orchestrator_vars.items():
            ...
            if var_info.get("type") == "int":
                try:
                    ...
                    if var_name == "MAX_STREAMS_PER_INSTANCE" and parsed_value <= 0:
                        result["issues"].append(
                            f"{var_name} must be positive, got: {parsed_value}"
                        )
                    elif var_name == "HEARTBEAT_TIMEOUT" and parsed_value < 30:
                        result["warnings"].append(
                            f"{var_name} is very low ({parsed_value}s), consider increasing for stability"
                        )
                    elif var_name == "REBALANCE_INTERVAL" and parsed_value < 10:
                        result["warnings"].append(
                            f"{var_name} is very low ({parsed_value}s), may cause excessive rebalancing"
                        )
            ...
            if var_info.get("valid_values") and value not in var_info["valid_values"]:
                result["issues"].append(
                    f"{var_name} has invalid value '{value}', valid values: {var_info['valid_values']}"
                )
    ...
    def _validate_timeout_config(self) -> Dict[str, Any]:
        ...
        timeout_vars = {
            "POSTGRES_STARTUP_TIMEOUT": {
                "default": "60",
                "description": "PostgreSQL startup timeout",
                "type": "int",
                "min": 30,
            },
            "REDIS_STARTUP_TIMEOUT": {
                "default": "30",
                "description": "Redis startup timeout",
                "type": "int",
                "min": 10,
            },
            "APP_STARTUP_TIMEOUT": {
                "default": "120",
                "description": "Application startup timeout",
                "type": "int",
                "min": 60,
            },
            "DB_MAX_RETRIES": {
                "default": "3",
                "description": "Database connection max retries",
                "type": "int",
                "min": 1,
            },
            "DB_RETRY_DELAY": {
                "default": "1.0",
                "description": "Database retry delay in seconds",
                "type": "float",
                "min": 0.1,
            },
        }
        ...
        for var_name, var_info in timeout_vars.items():
            ...
            try:
                ...
                if min_value is not None and parsed_value < min_value:
                    result["warnings"].append(
                        f"{var_name} is below recommended minimum ({min_value}): {parsed_value}"
                    )
                    result["recommendations"].append(
                        f"Consider increasing {var_name} to at least {min_value} for better reliability"
                    )

            except (ValueError, TypeError):
                result["issues"].append(
                    f"{var_name} is not a valid {var_info.get('type', 'value')}: {value}"
                )
            ...
            if path.exists():
                ...
                if path_info["required"]:
                    ...
                    try:
                        ...
                        result["warnings"].append(
                            f"Created missing directory: {path_str}"
                        )
                    except (PermissionError, OSError) as e:
                        result["issues"].append(
                            f"Cannot create directory {path_str}: {e}"
                        )
...
class DiagnosticFunctions:
    ...
    def _test_database_connectivity(self, db_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "connection refused" in error_msg:
                ...
                test_result["details"][
                    "suggestion"
                ] = "Check if PostgreSQL service is started and listening on the correct port"
                ...
                test_result["details"][
                    "likely_cause"
                ] = "Invalid hostname or DNS resolution failure"
                test_result["details"][
                    "suggestion"
                ] = "Verify DB_HOST setting and DNS configuration"
            else:
    ...
    def _test_database_authentication(
        self, db_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result["details"][
                "message"
            ] = f"Successfully authenticated as user '{db_config['user']}'"
    ...
    def _test_database_existence(self, db_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result["details"][
                "message"
            ] = f"Database '{db_config['database']}' exists and is accessible"
            ...
            if "database" in error_msg and "does not exist" in error_msg:
                ...
                test_result["details"][
                    "suggestion"
                ] = "Create the database or check DB_NAME environment variable"
                ...
                test_result["details"][
                    "message"
                ] = "Skipped due to connectivity or authentication issues"

        except Exception as e:
            test_result["status"] = "ERROR"
            test_result["error"] = str(e)
            self.logger.error(
                "Unexpected error in database existence test", exception=e
            )
    ...
    def _analyze_database_results(self, diagnosis: Dict[str, Any]):
            ...
            if test_result["status"] == "FAILED":
                ...
                if test_result["details"].get("suggestion"):
                    diagnosis["recommendations"].append(
                        test_result["details"]["suggestion"]
                    )
        ...
        if not diagnosis["recommendations"]:
            if diagnosis["status"] == "HEALTHY":
                diagnosis["recommendations"].append(
                    "Database connectivity is healthy - no action needed"
                )
            else:
                diagnosis["recommendations"].append(
    ...
    def diagnose_redis_connectivity(
        self, redis_host: str = "localhost", redis_port: int = 6379
    ) -> Dict[str, Any]:
        """
    ...
    def _test_redis_connectivity(self, host: str, port: int) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result["details"][
                "likely_cause"
            ] = "Redis server is not running or not accepting connections"
            test_result["details"][
                "suggestion"
            ] = "Check if Redis service is started and listening on the correct port"
            ...
            test_result["details"][
                "likely_cause"
            ] = "Redis server is responding slowly or overloaded"
            test_result["details"][
                "suggestion"
            ] = "Check Redis performance and server load"
            ...
            test_result["details"][
                "likely_cause"
            ] = "Unexpected error during Redis connectivity test"
            self.logger.error(
                "Unexpected error in Redis connectivity test", exception=e
            )
    ...
    def _test_redis_operations(self, host: str, port: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if retrieved_value and retrieved_value.decode() == test_value:
                ...
                test_result["details"][
                    "message"
                ] = "Redis operations returned unexpected results"
    ...
    def _test_redis_info(self, host: str, port: int) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result["status"] = "PASSED"
            test_result["details"]["redis_version"] = info.get(
                "redis_version", "unknown"
            )
            test_result["details"]["used_memory_human"] = info.get(
                "used_memory_human", "unknown"
            )
            test_result["details"]["connected_clients"] = info.get(
                "connected_clients", "unknown"
            )
            test_result["details"]["uptime_in_seconds"] = info.get(
    ...
    def _analyze_redis_results(self, diagnosis: Dict[str, Any]):
        ...
        if not diagnosis["recommendations"]:
            if diagnosis["status"] == "HEALTHY":
                diagnosis["recommendations"].append(
                    "Redis connectivity is healthy - no action needed"
                )
            else:
                diagnosis["recommendations"].append(
                    "Check Redis service status and configuration"
                )
    ...
    def generate_comprehensive_report(
        ...
        self.logger.info(
            f"Comprehensive diagnostic report completed",
            context={
