...
def debug_stream_filtering():
    ...
    assigned_stream_ids = [
        ...
        9,
        10,
    ]  # IDs numéricos do orquestrador
    assigned_stream_ids_str = [
        str(id) for id in assigned_stream_ids
    ...
    if assigned_streams_current:
        ...
        for stream in assigned_streams_current[:3]:
            print(
                f"     - {stream.get('name')} (ID: {stream.get('id', stream.get('name'))})"
            )
    else:
    ...
    if has_index_field > 0:
        index_types = set(
            type(stream.get("index")).__name__
            for stream in all_streams
            if "index" in stream
        )
        ...
        index_values = [
            stream.get("index") for stream in all_streams[:10] if "index" in stream
        ]
