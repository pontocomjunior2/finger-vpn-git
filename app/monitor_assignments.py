...
def monitor_assignments():
    ...
    try:
        while True:
            ...
            if show_status:
                print(f"[{timestamp}] DB: {db_count} | Instances: {instances} | Endpoint: {endpoint_count}")
                
                # Detectar inconsistências
                if isinstance(db_count, int) and isinstance(instances, dict) and isinstance(endpoint_count, int):
                    total_instance_streams = sum(instances.values())
                    
                    if db_count == 0 and total_instance_streams > 0:
                        print(f"  ⚠️  INCONSISTÊNCIA: DB vazio mas instances têm {total_instance_streams} streams")
                    
                    if db_count != endpoint_count:
                        print(f"  ⚠️  INCONSISTÊNCIA: DB ({db_count}) != Endpoint ({endpoint_count})")
