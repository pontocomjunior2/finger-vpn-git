        ...
        if response.status_code == 200:
            ...
            for instance in instances:
                ...
                if (
                    ...
                    and last_heartbeat.replace(tzinfo=None) > cutoff_time
                ):
...
def identify_rebalancing_needs(instances):
    ...
    return {
        ...
        "utilization": (
            (total_streams / total_capacity * 100) if total_capacity > 0 else 0
        ),
    }
...
def move_streams(streams, from_instance, to_instance):
    ...
    try:
        ...
        if moved_count > 0:
            ...
            conn.commit()
            print(
                f"   ✅ Movidos {moved_count} streams: {from_instance} → {to_instance}"
            )
            return True
        else:
...
def main():
    ...
    if final_analysis["overloaded"]:
        ...
        for instance in final_analysis["overloaded"]:
            print(
                f"   - {instance['server_id']}: {instance['current']}/{instance['max']} (+{instance['excess']})"
            )
    else:
