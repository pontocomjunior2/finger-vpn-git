...
class PerformanceValidator:
    ...
    async def measure_operation_performance(
        ...
        logger.info(
            f"Measuring performance for {operation_name} ({num_operations} operations)"
        )
        ...
        if concurrent:
            ...
            for i in range(num_operations):
                ...
                args = [
                    arg.format(i=i) if isinstance(arg, str) else arg
    ...
    async def test_instance_registration_performance(self) -> PerformanceMetrics:
        ...
        async def register_instance(server_id, ip, port, max_streams):
            return await self.orchestrator.register_instance(
    ...
    async def test_heartbeat_processing_performance(self) -> PerformanceMetrics:
        ...
        for i in range(10):
            await self.orchestrator.register_instance(
                f"heartbeat-worker-{i}", "127.0.0.1", 8000 + i, 20
            )
    ...
    async def test_stream_assignment_performance(self) -> PerformanceMetrics:
        ...
        for i in range(5):
            await self.orchestrator.register_instance(
                f"stream-worker-{i}", "127.0.0.1", 8100 + i, 50
            )
    ...
    async def test_concurrent_mixed_operations(self) -> PerformanceMetrics:
        ...
        for i in range(10):
            await self.orchestrator.register_instance(
                f"mixed-worker-{i}", "127.0.0.1", 8200 + i, 20
            )
        ...
        async def mixed_operation(operation_id: int):
            ...
            try:
                ...
                if operation_type == 0:
                    ...
                    await self.orchestrator.process_heartbeat(
                        f"mixed-worker-{operation_id % 10}", heartbeat_data
                    )
                elif operation_type == 1:
                    # Stream assignment
                    await self.orchestrator.assign_streams(
                        f"mixed-worker-{operation_id % 10}", 2
                    )
                    ...
                    await self.orchestrator.register_instance(
                        f"mixed-new-worker-{operation_id}",
                        "127.0.0.1",
                        9000 + operation_id,
                        20,
                    )
        ...
        tasks = [
            run_mixed_operation_with_semaphore(i) for i in range(num_mixed_operations)
        ]
        ...
        error_rate = (
            operations_failed / num_mixed_operations if num_mixed_operations > 0 else 0
        )
    ...
    async def test_high_load_performance(self) -> PerformanceMetrics:
        ...
        for i in range(num_instances):
            await self.orchestrator.register_instance(
                f"load-worker-{i}", "127.0.0.1", 7000 + i, 50
            )
        ...
        async def high_load_heartbeat(worker_id):
            ...
            return await self.orchestrator.process_heartbeat(
                f"load-worker-{worker_id}", heartbeat_data
            )
    ...
    def _generate_performance_report(
        ...
        total_successful = sum(
            metrics.successful_operations for metrics in test_results
        )
        total_failed = sum(metrics.failed_operations for metrics in test_results)

        overall_error_rate = (
            total_failed / total_operations if total_operations > 0 else 0
        )
        ...
        performance_thresholds = {
            "max_avg_response_time_ms": float(
                os.getenv("MAX_AVG_RESPONSE_TIME_MS", "500")
            ),
            "max_p95_response_time_ms": float(
                os.getenv("MAX_P95_RESPONSE_TIME_MS", "1000")
            ),
            "min_throughput_ops_per_sec": float(
                os.getenv("MIN_THROUGHPUT_OPS_PER_SEC", "10")
            ),
        ...
        for metrics in test_results:
            ...
            if (
                ...
                < performance_thresholds["min_throughput_ops_per_sec"]
            ):
                performance_issues.append(
...
async def main():
    ...
    try:
        ...
        if not await orchestrator.start_system():
            logger.error(
                "Failed to start integrated orchestrator for performance testing"
            )
            return 1
        ...
        logger.info(
            f"  Performance requirements met: {summary['performance_requirements_met']}"
        )
