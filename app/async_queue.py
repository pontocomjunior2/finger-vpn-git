...
class AsyncInsertQueue:
    ...
    def __init__(self, max_queue_size: int = 10000, batch_size: int = 100):
        ...
        self.batch_size = batch_size  # Aumentado para 100 para reduzir número de conexões
        self.worker_task: Optional[asyncio.Task] = None
        self.is_running = False
        self.processing_lock = asyncio.Lock()  # Lock para evitar processamento concorrente
        self.stats = {
            'total_queued': 0,
            'total_processed': 0,
            'total_failed': 0,
    ...
    async def _worker_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if "connection pool exhausted" in str(e).lower() or "timeout" in str(e).lower():
                ...
                import random
                backoff_time = current_delay * (0.5 + random.random())  # Adicionar jitter
                logger.warning(f"Aguardando {backoff_time:.2f}s antes de tentar novamente (erros consecutivos: {consecutive_errors})")
                await asyncio.sleep(backoff_time)
                
                # Se muitos erros consecutivos, forçar uma pausa maior
                if consecutive_errors >= max_consecutive_errors:
                    logger.warning(f"Muitos erros consecutivos ({consecutive_errors}). Pausando worker por 60 segundos.")
    ...
    async def _check_db_pool_health(self):
        ...
        try:
            ...
            if 'used_connections' in pool_stats and 'free_connections' in pool_stats:
                ...
                if usage_percent > 80:
                    logger.warning(f"Pool de conexões com uso elevado: {usage_percent:.1f}% ({used}/{max_conn})")
                elif usage_percent > 50:
                    logger.info(f"Pool de conexões com uso moderado: {usage_percent:.1f}% ({used}/{max_conn})")
                else:
                    logger.debug(f"Pool de conexões com uso normal: {usage_percent:.1f}% ({used}/{max_conn})")
    ...
    async def _collect_batch(self) -> list[InsertTask]:
        ...
        try:
            ...
            if len(batch) < 5 and self.queue.qsize() > 100:
                logger.debug(f"Aguardando mais tarefas chegarem (lote atual: {len(batch)}, fila: {self.queue.qsize()})")
                await asyncio.sleep(0.1)  # Pequena pausa para acumular mais tarefas
                
                # Tentar pegar mais algumas tarefas após a pausa
                try:
                    remaining = min(current_batch_size - len(batch), 10)  # Limitar a 10 tarefas adicionais
        ...
        if batch:
            logger.debug(f"Coletado lote de {len(batch)} tarefas (tamanho adaptativo: {current_batch_size})")
    ...
    async def _insert_single_task(self, cursor, task: InsertTask):
        ...
        try:
            ...
            if result and result[0]:
                logger.debug(f"Inserção bem-sucedida: ID={result[0]} - {data.get('name')} - {data.get('song_title')}")
            else:
                logger.debug(f"Registro já existente (ignorado): {data.get('name')} - {data.get('song_title')}")
            ...
            if "duplicate key" in error_msg:
                logger.debug(f"Registro duplicado ignorado: {data.get('name')} - {data.get('song_title')}")
