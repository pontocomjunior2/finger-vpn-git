...
class IntegrationValidator:
    ...
    async def validate_component_integration(
        ...
        try:
            ...
            for component_name in required_components:
                ...
                if component is None:
                    ...
                    validation_result["errors"].append(
                        f"Component {component_name} not initialized"
                    )
                else:
            ...
            validation_result["details"]["health_ratio"] = (
            ...
            if healthy_components < total_components:
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"Only {healthy_components}/{total_components} components are healthy"
                )

            logger.info(
                f"Component integration validation: {'PASSED' if validation_result['success'] else 'FAILED'}"
            )
    ...
    async def validate_system_startup(
        ...
        try:
            ...
            if orchestrator.status != IntegrationStatus.RUNNING:
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"System status is {orchestrator.status.value}, expected RUNNING"
                )

            # Check background tasks
            validation_result["details"]["background_tasks_count"] = len(
                orchestrator.background_tasks
            )
            ...
            logger.info(
                f"System startup/shutdown validation: {'PASSED' if validation_result['success'] else 'FAILED'}"
            )

        except Exception as e:
            validation_result["success"] = False
            validation_result["errors"].append(
                f"System startup/shutdown validation failed: {str(e)}"
            )
    ...
    async def validate_end_to_end_operations(
        self, db_config: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result["details"]["instance_registration"] = (
            ...
            if registration_result["status"] != "success":
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"Instance registration failed: {registration_result.get('error')}"
                )
            ...
            heartbeat_result = await orchestrator.process_heartbeat(
                "validation-worker-1", heartbeat_data
            )

            validation_result["details"]["heartbeat_processing"] = (
            ...
            if heartbeat_result["status"] != "success":
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"Heartbeat processing failed: {heartbeat_result.get('error')}"
                )

            # Test stream assignment
            assignment_result = await orchestrator.assign_streams(
                "validation-worker-1", 5
            )

            validation_result["details"]["stream_assignment"] = (
            ...
            if assignment_result["status"] != "success":
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"Stream assignment failed: {assignment_result.get('error')}"
                )
            ...
            validation_result["details"]["active_instances"] = status.get(
                "active_instances", 0
            )
            validation_result["details"]["total_streams"] = status.get(
                "total_streams", 0
            )

            logger.info(
                f"End-to-end operations validation: {'PASSED' if validation_result['success'] else 'FAILED'}"
            )

        except Exception as e:
            validation_result["success"] = False
            validation_result["errors"].append(
                f"End-to-end operations validation failed: {str(e)}"
            )
    ...
    async def validate_failure_scenarios(
        self, db_config: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await orchestrator.register_instance(
                "failure-worker-1", "127.0.0.1", 8001, 20
            )
            await orchestrator.register_instance(
                "failure-worker-2", "127.0.0.1", 8002, 20
            )
            ...
            failure_result = await orchestrator.handle_instance_failure(
                "failure-worker-1"
            )

            validation_result["details"]["failure_handling"] = (
                failure_result["status"] == "success"
            )
            validation_result["details"]["recovery_performed"] = failure_result.get(
            ...
            if failure_result["status"] != "success":
                validation_result["success"] = False
                validation_result["errors"].append(
                    f"Instance failure handling failed: {failure_result.get('error')}"
                )

            logger.info(
                f"Failure scenarios validation: {'PASSED' if validation_result['success'] else 'FAILED'}"
            )

        except Exception as e:
            validation_result["success"] = False
            validation_result["errors"].append(
                f"Failure scenarios validation failed: {str(e)}"
            )
        ...
        try:
            ...
            for i in range(10):
                ...
                if result["status"] != "success":
                    validation_result["success"] = False
                    validation_result["errors"].append(
                        f"Registration failed for worker {i}"
                    )
            ...
            max_allowed_registration_time = float(
                os.getenv("MAX_REGISTRATION_TIME", "2.0")
            )
            ...
            logger.info(
                f"Performance requirements validation: {'PASSED' if validation_result['success'] else 'FAILED'}"
            )

        except Exception as e:
            validation_result["success"] = False
            validation_result["errors"].append(
                f"Performance requirements validation failed: {str(e)}"
            )
    ...
    async def run_comprehensive_validation(
        ...
        try:
            await orchestrator.start_system()
            component_validation = await self.validate_component_integration(
                orchestrator
            )
...
async def main():
    ...
    try:
        ...
        if summary["failed_validations"] > 0:
            ...
            for validation in validation_report["validations"]:
                if not validation["success"]:
                    logger.error(
                        f"  - {validation['test_name']}: {', '.join(validation['errors'])}"
                    )
        ...
        overall_success = (
            ...
            and test_report["summary"]["failed_tests"] == 0
        )

        if overall_success:
            logger.info(
                "✓ All integration validation and testing completed successfully"
            )
            return 0
        else:
