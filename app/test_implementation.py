...
async def mock_acquire_stream_lock(stream_id: str, server_id: int, timeout: int = 30) -> bool:
...
async def test_consistent_hashing():
    ...
    for total_servers in server_counts:
        ...
        for server_id, count in server_distribution.items():
            print(f"  Server {server_id}: {count} streams ({count/len(test_streams)*100:.1f}%)")
        
        # Verify no duplicate processing
        total_assigned = sum(server_distribution.values())
        assert total_assigned == len(test_streams), f"Expected {len(test_streams)} streams, got {total_assigned}"
...
async def test_distributed_locking():
    ...
    test_stream = {'id': 'test_stream_1', 'name': 'Test Stream', 'url': 'http://test.com'}
...
async def main():
    ...
    print(f"Current configuration: SERVER_ID={SERVER_ID}, TOTAL_SERVERS={TOTAL_SERVERS}")
    print("=" * 50)
