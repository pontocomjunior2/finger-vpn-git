    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for instance in instances:
                print(f"  - Servidor {instance['server_id']}: {instance['current_streams']}/{instance['max_streams']} streams")
        else:
        ...
        for instance in instances:
            if instance['current_streams'] > 0:
                ...
                assigned_streams = [a['stream_id'] for a in assignments if a['server_id'] == server_id]
                ...
                if assigned_streams:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"   ✅ {result.get('count', 0)} streams liberados do servidor {server_id}")
                    else:
                        print(f"   ❌ Erro ao liberar streams do servidor {server_id}: {response.status_code}")
        ...
        for instance in instances:
            if instance['status'] == 'active':
                ...
                print(f"   Solicitando {max_streams} streams para servidor {server_id}...")
                
                request_data = {
                    "server_id": server_id,
                    "requested_count": max_streams
                }
                ...
                if response.status_code == 200:
                    ...
                    print(f"   ✅ {assigned_count} streams atribuídos ao servidor {server_id}")
                else:
                    print(f"   ❌ Erro ao solicitar streams para servidor {server_id}: {response.status_code}")
