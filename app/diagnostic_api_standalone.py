...
class StandaloneDiagnosticAPI:
    ...
    async def _get_instances_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"total_instances": 0, "instances": [], "error": str(e)}
        ...
        for check in health_checks:
            if check.status == HealthStatus.CRITICAL:
                recommendations.append(
                    f"CRITICAL: Fix {check.component} issues - {check.message}"
                )
...
def create_standalone_diagnostic_api(
...
async def run_standalone_diagnostic_server(
    host: str = "0.0.0.0", port: int = 8081, db_config: Optional[Dict] = None
):
