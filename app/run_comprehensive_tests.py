...
class TestSuiteRunner:
    ...
    def run_pytest_suite(self, test_file: str, test_name: str, 
    ...
    def generate_summary_report(self) -> Dict:
        ...
        # Categorize results
        unit_test_results = {k: v for k, v in self.test_results.items() 
                           if 'unit' in k or 'error_scenarios' in k}
        integration_test_results = {k: v for k, v in self.test_results.items() 
                                  if 'orchestrator_worker' in k}
        load_test_results = {k: v for k, v in self.test_results.items() 
                           if 'load' in k or 'performance' in k}
        existing_test_results = {k: v for k, v in self.test_results.items() 
                               if 'existing' in k}
        ...
        summary = {
            ...
            'category_summary': {
                'unit_tests': {
                    ...
                    'failed': sum(1 for r in unit_test_results.values() if not r['passed'])
                },
                'integration_tests': {
                    'total': len(integration_test_results),
                    'passed': sum(1 for r in integration_test_results.values() if r['passed']),
                    'failed': sum(1 for r in integration_test_results.values() if not r['passed'])
                },
                'load_tests': {
                    'total': len(load_test_results),
                    'passed': sum(1 for r in load_test_results.values() if r['passed']),
                    'failed': sum(1 for r in load_test_results.values() if not r['passed'])
                },
                'existing_tests': {
                    'total': len(existing_test_results),
                    'passed': sum(1 for r in existing_test_results.values() if r['passed']),
                    'failed': sum(1 for r in existing_test_results.values() if not r['passed'])
                }
            },
    ...
    def print_summary_report(self, summary: Dict):
        ...
        for category, stats in summary['category_summary'].items():
            ...
            logger.info(f"  {category_name}: {stats['passed']}/{stats['total']} ({success_rate:.1f}%)")
        
        if summary['failed_test_details']:
            logger.info("\nFailed Test Details:")
            for test_name, details in summary['failed_test_details'].items():
                logger.info(f"  ✗ {test_name} ({details['file']}) - {details['execution_time']:.2f}s")
                if details['stderr']:
        ...
        if info['success_rate'] >= 90:
            ...
            logger.info("❌ CRITICAL: Test suite has high failure rate. Immediate attention required.")
    
    def save_detailed_report(self, summary: Dict, filename: str = "comprehensive_test_report.json"):
    ...
    def run_full_test_suite(self, include_existing: bool = True, 
                          include_load_tests: bool = True) -> Dict:
...
def main():
    ...
    parser.add_argument('--no-existing', action='store_true', 
                       help='Skip existing test suites')
    parser.add_argument('--no-load-tests', action='store_true',
                       help='Skip load testing framework')
    ...
    parser.add_argument('--app-dir', default='app',
                       help='Application directory (default: app)')
