...
try:
    ...
    cursor.execute(
        "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'streams' ORDER BY ordinal_position"
    )
    ...
    if not has_id_column:
        ...
        if has_index_column:
            print(
                "   ✅ Tabela tem coluna 'index' - esta deveria ser usada pelo orquestrador"
            )
        ...
        try:
            ...
            print(
                f"   Query executada com sucesso: {len(all_streams)} streams encontrados"
            )
    ...
    if active_assignments > 0:
        cursor.execute(
            "SELECT stream_id FROM orchestrator_stream_assignments WHERE status = 'active' LIMIT 10"
        )
