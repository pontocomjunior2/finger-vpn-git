...
def validate_endpoint_exists(
    source_code: str, endpoint_path: str, method: str = "GET"
) -> Dict[str, Any]:
    """
        ...
        for i, line in enumerate(lines):
            if decorator_pattern in line:
                ...
                for j in range(i + 1, min(i + 5, len(lines))):
                    ...
                    if func_line.startswith("async def ") or func_line.startswith(
                        "def "
                    ):
                        result["found"] = True
                        ...
                        while (
                            ...
                            and not lines[docstring_start].strip()
                        ):
                            docstring_start += 1

                        if (
                            docstring_start < len(lines)
                            and '"""' in lines[docstring_start]
                        ):
                            # Extract docstring
                            docstring_lines = []
                            in_docstring = False
                            ...
                            if (
                                "requirements:" in docstring_text
                                or "requirement" in docstring_text
                            ):
                ...
                if func_pattern in line:
                    ...
                    for j in range(i + 1, min(i + 10, len(lines))):
                        if '"""' in lines[j]:
                            ...
                            docstring_end = j + 1
                            while (
                                docstring_end < len(lines)
                                and '"""' not in lines[docstring_end]
                            ):
                                docstring_end += 1
