    ...
    try:
        ...
        if instances:
            for instance in instances:
                ...
                if instance["current_streams"] > instance["max_streams"]:
                    print(
                        f"     ⚠️  PROBLEMA: Instância excede limite! ({instance['current_streams']} > {instance['max_streams']})"
                    )
                else:
                    print(
                        f"     ✅ OK: Dentro do limite ({instance['current_streams']} <= {instance['max_streams']})"
                    )
                print()
        else:
        ...
        if assignments:
            for assignment in assignments:
                print(
                    f"   Instância {assignment['server_id']}: {assignment['assigned_streams']} streams atribuídos"
                )

                # Comparar com a instância registrada
                instance = next(
                    (i for i in instances if i["server_id"] == assignment["server_id"]),
                    None,
                )
                if instance:
                    if assignment["assigned_streams"] != instance["current_streams"]:
                        print(
                            f"     ⚠️  INCONSISTÊNCIA: Assignments ({assignment['assigned_streams']}) != Current Streams ({instance['current_streams']})"
                        )
                    else:
        ...
        if all_within_limits:
            print(
                "   ✅ SUCESSO: Todas as instâncias estão respeitando seus limites de MAX_STREAMS"
            )
        else:
            print(
                "   ❌ PROBLEMA: Algumas instâncias excedem seus limites de MAX_STREAMS"
            )
