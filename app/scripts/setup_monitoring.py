...
class MonitoringSetup:
                    ...
                    "scrape_configs": [
                        {
                            ...
                            "static_configs": [
                                {
                                    "targets": [
                                        f"orchestrator:{self.config.orchestrator.port}"
                                    ]
                                }
                            ],
    ...
    def _setup_grafana(self) -> bool:
        ...
        try:
            ...
            with open(
                self.grafana_dir / "provisioning" / "datasources" / "prometheus.yml",
                "w",
            ) as f:
            ...
            with open(
                self.grafana_dir / "provisioning" / "dashboards" / "orchestrator.yml",
                "w",
            ) as f:
...
def main():
    ...
    parser.add_argument(
        "--setup", action="store_true", help="Setup monitoring infrastructure"
    )
    parser.add_argument(
        "--start", action="store_true", help="Start monitoring services"
    )
