...
class OrchestratorDeployment:
    ...
    def deploy(self) -> bool:
        ...
        logger.info(
            f"Starting orchestrator deployment {self.deployment_id} for {self.environment}"
        )
        ...
        try:
            ...
            if not self.skip_validation and not self._validate_deployment():
                if not self.force_deploy:
                    logger.error(
                        "Pre-deployment validation failed. Use --force to override."
                    )
                    return False
    ...
    def _verify_deployment(self) -> bool:
        ...
        try:
            ...
            compose_file = (
                f"deployment_{self.environment}/docker-compose.{self.environment}.yml"
            )
...
def main():
    ...
    parser.add_argument(
        "--skip-monitoring", action="store_true", help="Skip monitoring setup"
    )
    parser.add_argument(
