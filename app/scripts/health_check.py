...
class HealthChecker:
    ...
    async def check_system_health(self) -> SystemHealth:
        ...
        for component, config in self.endpoints.items():
            try:
                if "url" in config:
                    health = await self._check_http_endpoint(component, config["url"], config["timeout"])
    ...
    async def _check_http_endpoint(self, component: str, url: str, timeout: int) -> ComponentHealth:
    ...
    def _calculate_overall_health(self, component_healths: List[ComponentHealth]) -> HealthStatus:
        ...
        unhealthy_count = sum(1 for h in component_healths if h.status == HealthStatus.UNHEALTHY)
        degraded_count = sum(1 for h in component_healths if h.status == HealthStatus.DEGRADED)
    ...
    async def wait_for_healthy(self, timeout: int = 300, interval: int = 10) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            self.logger.info(f"System status: {health.overall_status.value}, waiting {interval}s...")
            
            # Log unhealthy components
            for comp in health.components:
                if comp.status != HealthStatus.HEALTHY:
                    self.logger.warning(f"  {comp.component}: {comp.status.value} - {comp.error}")
    ...
    def check_liveness(self) -> bool:
        ...
        try:
            ...
            return response.status_code in [200, 503]  # 503 might indicate degraded but alive
        except:
            return False
...
async def main():
    ...
    parser.add_argument("--interval", "-i", type=int, default=10,
                       help="Check interval when waiting")
    parser.add_argument("--readiness", action="store_true",
                       help="Quick readiness check")
    parser.add_argument("--liveness", action="store_true",
                       help="Quick liveness check")
    ...
    exit_code = 0 if health.overall_status in [HealthStatus.HEALTHY, HealthStatus.DEGRADED] else 1
    sys.exit(exit_code)
