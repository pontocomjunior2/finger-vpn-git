...
class DeploymentConfigurator:
    ...
    def _generate_grafana_config(self, monitoring_dir: Path):
        ...
        with open(grafana_dir / "provisioning" / "datasources" / "prometheus.yml", 'w') as f:
    ...
    def validate_deployment_config(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run([
                "docker-compose", "-f", str(self.output_dir / f"docker-compose.{self.environment}.yml"), "config"
...
def main():
    ...
    if success:
        logger.info(f"Deployment configuration completed successfully for {args.environment}")
