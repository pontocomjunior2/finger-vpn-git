...
class DeploymentManager:
    ...
    def __init__(self, config: DeploymentConfig):
        ...
        self.health_endpoints = {"orchestrator": "/health", "monitoring": "/api/health"}
    ...
    def deploy(self) -> bool:
        ...
        logger.info(
            f"Starting deployment {self.deployment_id} for environment: {self.config.environment}"
        )
    ...
    def _pre_deployment_checks(self) -> bool:
        ...
        env_config = self.config_manager.load_environment_config(
    ...
    def _deploy_services(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["docker-compose", "-f", compose_file, "down"], check=False
            )  # Don't fail if services weren't running
        ...
        while time.time() - start_time < self.config.health_check_timeout:
            ...
            for service in self.config.services:
                ...
                if not health_result.healthy:
                    all_healthy = False
                    logger.info(
                        f"Service {service} not healthy yet: {health_result.error}"
                    )
                else:
                    logger.info(
                        f"Service {service} is healthy (response time: {health_result.response_time:.2f}s)"
                    )
    ...
    def _check_service_health(self, service: str) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(service=service, healthy=False, error=str(e))
    ...
    def _run_post_deployment_tests(self) -> bool:
        ...
        try:
            ...
            test_script = (
                Path(__file__).parent.parent / "test_integration_orchestrator_worker.py"
            )
            if test_script.exists():
                result = subprocess.run(
...
def main():
    ...
    parser.add_argument(
        "--environment", "-e", default="production", help="Environment to deploy"
    )
    parser.add_argument(
        "--services",
        "-s",
    ...
    parser.add_argument(
        "--no-backup", action="store_true", help="Disable backup before deployment"
    )
    parser.add_argument(
        "--no-tests", action="store_true", help="Skip pre and post deployment tests"
    )
    parser.add_argument(
        "--health-timeout",
        type=int,
