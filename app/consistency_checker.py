...
class ConsistencyChecker:
    ...
    async def verify_stream_assignments(self) -> ConsistencyReport:
        ...
        try:
            ...
            recommendations = self._generate_recommendations(stream_issues, instance_issues)
            ...
            logger.info(f"Consistency check completed. Score: {consistency_score:.2f}, "
    ...
    async def detect_orphaned_streams(self) -> List[StreamAssignmentIssue]:
        ...
        return await self._detect_orphaned_streams(orchestrator_assignments, worker_assignments)
    
    async def resolve_conflicts(self, conflicts: List[StreamAssignmentIssue]) -> List[RecoveryResult]:
        """
        ...
        for conflict in conflicts:
            try:
                ...
                self.recovery_attempts[conflict_key] = self.recovery_attempts.get(conflict_key, 0) + 1
                
            except Exception as e:
                logger.error(f"Failed to resolve conflict for stream {conflict.stream_id}: {e}")
    ...
    async def synchronize_instance_state(self, instance_id: str) -> RecoveryResult:
        ...
        try:
            # Get current states
            orchestrator_state = await self._get_instance_state_from_orchestrator(instance_id)
            ...
            if not orchestrator_state and worker_state:
                ...
                result = await self._force_reregister_instance(instance_id, worker_state)
            elif orchestrator_state and not worker_state:
                # Orchestrator has instance but worker is gone - clean up
                result = await self._cleanup_orphaned_instance(instance_id, orchestrator_state)
            else:
    ...
    async def auto_recover_inconsistencies(self, report: ConsistencyReport) -> List[RecoveryResult]:
        """
        ...
        critical_issues = [issue for issue in report.stream_issues if issue.severity == "critical"]
        other_issues = [issue for issue in report.stream_issues if issue.severity != "critical"]
        ...
        for issue in critical_issues:
            if self._should_attempt_recovery(issue):
                try:
                    ...
                    logger.error(f"Auto-recovery failed for critical issue {issue.stream_id}: {e}")
        ...
        for issue in other_issues:
            if self._should_attempt_recovery(issue):
                try:
                    ...
                    logger.error(f"Auto-recovery failed for issue {issue.stream_id}: {e}")
        
        # Process instance issues
        for instance_issue in report.instance_issues:
            try:
                result = await self.synchronize_instance_state(instance_issue.instance_id)
                results.append(result)
            except Exception as e:
                logger.error(f"Instance sync failed for {instance_issue.instance_id}: {e}")
        
        successful_recoveries = len([r for r in results if r.success])
        logger.info(f"Auto-recovery completed. {successful_recoveries}/{len(results)} successful")
    ...
    async def _detect_orphaned_streams(self, orchestrator_assignments: Dict[int, str], 
                                     worker_assignments: Dict[str, List[int]]) -> List[StreamAssignmentIssue]:
        ...
        for stream_id, server_id in orchestrator_assignments.items():
            if stream_id not in worker_streams:
                issues.append(StreamAssignmentIssue(
                    stream_id=stream_id,
    ...
    async def _detect_duplicate_assignments(self, orchestrator_assignments: Dict[int, str],
                                          worker_assignments: Dict[str, List[int]]) -> List[StreamAssignmentIssue]:
    ...
    async def _detect_unauthorized_streams(self, orchestrator_assignments: Dict[int, str],
                                         worker_assignments: Dict[str, List[int]]) -> List[StreamAssignmentIssue]:
            ...
            async with self.db_manager.get_connection() as conn:
                ...
                for instance_id, state in orchestrator_instances.items():
                    if state['last_heartbeat']:
                        ...
                        if last_heartbeat < stale_threshold and state['status'] == 'active':
                            issues.append(InstanceStateIssue(
    ...
    def _calculate_consistency_score(self, total_streams: int, issues_count: int) -> float:
        ...
        critical_penalty = 0.1 * len([issue for issue in getattr(self, '_current_issues', []) 
                                    if getattr(issue, 'severity', '') == 'critical'])
        
        return max(0.0, base_score - critical_penalty)
    
    def _generate_recommendations(self, stream_issues: List[StreamAssignmentIssue], 
                                instance_issues: List[InstanceStateIssue]) -> List[str]:
        """Generate actionable recommendations based on detected issues."""
        recommendations = []
        
        # Stream-related recommendations
        orphaned_count = len([i for i in stream_issues if i.issue_type == ConsistencyStatus.ORPHANED])
        duplicate_count = len([i for i in stream_issues if i.issue_type == ConsistencyStatus.DUPLICATE])
        unauthorized_count = len([i for i in stream_issues if i.issue_type == ConsistencyStatus.INCONSISTENT])
        
        if orphaned_count > 0:
            recommendations.append(f"Reassign {orphaned_count} orphaned streams to active workers")
        
        if duplicate_count > 0:
            recommendations.append(f"Resolve {duplicate_count} duplicate stream assignments immediately")
        
        if unauthorized_count > 0:
            recommendations.append(f"Synchronize {unauthorized_count} unauthorized streams with orchestrator")
        
        # Instance-related recommendations
        stale_instances = len([i for i in instance_issues if 'stale heartbeat' in i.description])
        if stale_instances > 0:
            recommendations.append(f"Check connectivity for {stale_instances} instances with stale heartbeats")
        
        # General recommendations
        if len(stream_issues) > 10:
            recommendations.append("Consider running full system rebalancing due to high inconsistency count")
    ...
    async def _resolve_single_conflict(self, conflict: StreamAssignmentIssue) -> RecoveryResult:
    ...
    async def _resolve_duplicate_assignment(self, conflict: StreamAssignmentIssue) -> RecoveryResult:
    ...
    async def _resolve_orphaned_stream(self, conflict: StreamAssignmentIssue) -> RecoveryResult:
    ...
    async def _resolve_unauthorized_stream(self, conflict: StreamAssignmentIssue) -> RecoveryResult:
    ...
    async def _auto_recover_issue(self, issue: StreamAssignmentIssue) -> RecoveryResult:
        ...
        logger.info(f"Attempting auto-recovery for stream {issue.stream_id}, type: {issue.issue_type}")
        
        return await self._resolve_single_conflict(issue)
    
    async def _get_instance_state_from_orchestrator(self, instance_id: str) -> Optional[Dict]:
    ...
    async def _force_reregister_instance(self, instance_id: str, worker_state: Dict) -> RecoveryResult:
        ...
        try:
            async with self.db_manager.get_connection() as conn:
                ...
                """, (instance_id, worker_state.get('active_streams', 0), datetime.now().isoformat()))
    ...
    async def _cleanup_orphaned_instance(self, instance_id: str, orchestrator_state: Dict) -> RecoveryResult:
        """Clean up orphaned instance from orchestrator."""
        try:
            async with self.db_manager.get_connection() as conn:
                # Remove instance
                await conn.execute("DELETE FROM instances WHERE server_id = ?", (instance_id,))
    ...
    async def _sync_instance_differences(self, instance_id: str, 
                                       orchestrator_state: Dict, worker_state: Dict) -> RecoveryResult:
        ...
        try:
            ...
            if orchestrator_streams != worker_streams:
                differences.append(f"Stream count mismatch: orchestrator={orchestrator_streams}, worker={worker_streams}")
...
class ConsistencyMonitor:
    ...
    def __init__(self, consistency_checker: ConsistencyChecker, check_interval: int = 300):
    ...
    async def start_monitoring(self):
        ...
        logger.info(f"Starting consistency monitoring with {self.check_interval}s interval")
    ...
    async def _perform_check_cycle(self):
        ...
        if not report.is_healthy and len(report.stream_issues) > 0:
            logger.warning(f"Consistency issues detected: {len(report.stream_issues)} issues, score: {report.consistency_score:.2f}")
            
            # Attempt auto-recovery for non-critical issues
            non_critical_issues = [issue for issue in report.stream_issues if issue.severity != "critical"]
            ...
            logger.debug(f"System consistency healthy: score {report.consistency_score:.2f}")
