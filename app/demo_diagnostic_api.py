        ...
        try:
            ...
            if inconsistencies:
                for inc in inconsistencies:
                    ...
                    if inc.metadata:
                        print(f"    Details: {json.dumps(inc.metadata, indent=6, default=str)}")
                    print()
            else:
...
async def demo_detailed_status_reporting():
    ...
    if instances:
        ...
        for instance in instances[:3]:  # Show first 3 instances
            print(f"    • {instance.get('server_id', 'unknown')}: "
...
async def demo_recommendations_and_actions():
    ...
    if recommendations:
        for i, rec in enumerate(recommendations, 1):
            priority_icon = "🚨" if "CRITICAL" in rec else "⚠️" if "WARNING" in rec else "ℹ️"
...
async def main():
    ...
    try:
        ...
        print("All requirements (6.1, 6.2, 6.3, 6.4) have been successfully implemented.")
        print()
