...
class DeadlockDetector:
    ...
    def record_deadlock(self, error: Exception, context: str, query: str = None):
        ...
        with self._lock:
            ...
            self.deadlock_patterns[error_key] = (
                self.deadlock_patterns.get(error_key, 0) + 1
            )
...
class EnhancedDatabaseManager:
    ...
    def _create_pool(self):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if missing_configs:
                    raise ValueError(
                        f"Missing database configurations: {missing_configs}"
                    )

                # Create pool with enhanced settings
                self.pool = psycopg2.pool.ThreadedConnectionPool(
                    minconn=self.minconn, maxconn=self.maxconn, **db_config
                )

                logger.info(
                    f"Enhanced database pool created successfully (attempt {attempt + 1})"
                )
                return
                ...
                if attempt == self.max_retries - 1:
                    logger.error(
                        f"Failed to create database pool after {self.max_retries} attempts: {e}"
                    )
                    raise

                delay = self.base_retry_delay * (2**attempt) + random.uniform(0, 0.1)
                logger.warning(
                    f"Pool creation attempt {attempt + 1} failed, retrying in {delay:.2f}s: {e}"
                )
                time.sleep(delay)
    ...
    def _configure_connection(self, conn):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    f"SET idle_in_transaction_session_timeout = '{self.connection_timeout * 1000}'"
                )
                ...
                cursor.execute(
                    f"SET application_name = 'enhanced_fingerv7_{os.getenv('SERVER_ID', '1')}'"
                )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                conn = await asyncio.wait_for(
                    asyncio.to_thread(self.pool.getconn), timeout=connection_timeout
                )
                    ...
                    try:
                        ...
                        try:
                            if conn and not conn.closed:
                                ...
                                logger.warning(
                                    f"Connection already closed in context: {context}"
                                )
                        except Exception as e:
                ...
                logger.warning(
                    f"Connection timeout in {context}, retrying in {delay:.2f}s"
                )
                ...
                if attempt == self.max_retries - 1:
                    logger.error(
                        f"Failed to get connection after {self.max_retries} attempts in {context}: {e}"
                    )
                    raise

                delay = self.deadlock_detector.get_retry_delay(attempt)
                logger.warning(
                    f"Connection attempt {attempt + 1} failed in {context}, retrying in {delay:.2f}s: {e}"
                )
                await asyncio.sleep(delay)
    ...
    async def get_transaction(self, context: str = "unknown", timeout: float = None):
        ...
        async with self.get_connection(context, timeout) as conn:
            ...
            try:
                ...
                self.transaction_monitor.complete_transaction(
                ...
                try:
                    ...
                    if self.deadlock_detector.is_deadlock_error(e):
                        self.transaction_monitor.complete_transaction(
                            transaction_id, TransactionStatus.DEADLOCK
                        )
                    else:
                        self.transaction_monitor.complete_transaction(
    ...
    async def execute_with_retry(
        self, operation: Callable, context: str = "unknown", max_retries: int = None
    ) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"Operation failed after {max_retries} attempts in {context}: {e}"
                    )
                    raise
                ...
                logger.warning(
                    f"Operation attempt {attempt + 1} failed in {context}, retrying in {delay:.2f}s: {e}"
                )
                await asyncio.sleep(delay)
                self.metrics.retry_count += 1
    ...
    def get_health_status(self) -> Dict:
        ...
        return {
            ...
            "error_patterns": {
                ...
                for pattern_key, pattern in list(self.error_patterns.items())[
                    :10
                ]  # Top 10 patterns
            },
        }
