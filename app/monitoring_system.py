...
class PatternDetector:
    ...
    def _analyze_pattern(
        self, pattern: FailurePattern, config: Dict, current_time: datetime
        ...
        recent_events = [
            event for event in self.failure_events if event.timestamp >= cutoff_time
        ]
    ...
    def _detect_cascading_failures(
        self, events: List[FailureEvent], config: Dict
    ) -> Optional[Dict[str, Any]]:
        ...
        failing_sources = [
            source for source, count in source_failures.items() if count >= 2
        ]

        if len(failing_sources) >= 3:  # Multiple sources failing
            failure_rate = (
                len(events) / len(self.failure_events) if self.failure_events else 0
            )
    ...
    def _detect_resource_exhaustion(
        ...
        resource_events = [
            event
            for event in events
            if "resource_usage" in event.metadata
            or "memory" in event.failure_type.lower()
            or "cpu" in event.failure_type.lower()
        ]
    ...
    def _detect_network_partition(
        ...
        network_events = [
            ...
            or "connection" in event.failure_type.lower()
    ...
    def _detect_database_contention(
        self, events: List[FailureEvent], config: Dict
    ) -> Optional[Dict[str, Any]]:
        ...
        db_events = [
            event
            for event in events
            if "database" in event.failure_type.lower()
            or "deadlock" in event.failure_type.lower()
            or "lock" in event.failure_type.lower()
        ]
    ...
    def _detect_heartbeat_degradation(
        self, events: List[FailureEvent], config: Dict
    ) -> Optional[Dict[str, Any]]:
        ...
        heartbeat_events = [
            event
            for event in events
            if "heartbeat" in event.failure_type.lower()
            or "health" in event.failure_type.lower()
        ]
    ...
    def _detect_load_imbalance(
        ...
        load_events = [
            event
            for event in events
            if "load" in event.failure_type.lower()
            or "balance" in event.failure_type.lower()
            or "capacity" in event.failure_type.lower()
        ]
...
class MetricsCollector:
    ...
    def record_throughput(self, operation: str, requests_per_second: float):
        ...
        self.record_metric(
            f"throughput_{operation}", requests_per_second, MetricType.GAUGE
        )
    ...
    def record_resource_usage(self, resource: str, usage_percent: float):
        ...
        self.record_metric(
            f"resource_usage_{resource}", usage_percent, MetricType.GAUGE
        )
        ...
        if len(self.performance_metrics.resource_usage[resource]) > 100:
            self.performance_metrics.resource_usage[resource] = (
                self.performance_metrics.resource_usage[resource][-100:]
            )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_error_rate": (
                statistics.mean(self.performance_metrics.error_rates)
                if self.performance_metrics.error_rates
                else 0
            ),
            "avg_throughput": (
                statistics.mean(self.performance_metrics.throughput)
...
class MonitoringSystem:
    ...
    async def _handle_detected_pattern(
        self, pattern: FailurePattern, info: Dict[str, Any]
    ):
    ...
    async def _evaluate_single_rule(self, rule_name: str, rule_config: Dict[str, Any]):
        ...
        if (
            violation_start
            and (current_time - violation_start).total_seconds() >= duration
        ):
    ...
    async def _monitor_system_health(self):
        ...
        try:
            ...
            if len(recent_failures) > 10:  # More than 10 failures in 5 minutes
                self.alert_manager.create_alert(
                    ...
                    metadata={
                        ...
                        "time_window": 300,
                        "failure_sources": list(
                            set(event.source for event in recent_failures)
                        ),
                    },
                )
            ...
            for (
                resource,
                usage_history,
            ) in self.metrics_collector.performance_metrics.resource_usage.items():
    ...
    def get_system_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts": {
                ...
                "warning_count": len(
                    self.alert_manager.get_active_alerts(AlertSeverity.WARNING)
                ),
            ...
            "operations": {
                "success_count": sum(
                    count
                    for name, count in self.operation_counters.items()
                    if "success" in name
                ),
                "error_count": sum(
                    count
    ...
    async def _check_database_health(self):
        ...
        try:
            ...
            self.metrics_collector.record_response_time(
                "database_health_check", response_time
            )
