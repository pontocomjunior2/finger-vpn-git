    ...
    def detect_patterns(self) -> List[tuple]:
        ...
        if len(recent_events) >= 5:
            ...
            if len(sources) >= 3:
                ...
                patterns.append((FailurePattern.CASCADING_FAILURES, {
        ...
        network_events = [
            ...
            if 'network' in event.failure_type.lower() or 'timeout' in event.failure_type.lower()
        ]
        
        if len(network_events) >= 3:
            confidence = len(network_events) / len(recent_events) if recent_events else 0
            if confidence >= 0.6:
                patterns.append((FailurePattern.NETWORK_PARTITION, {
                    'confidence': confidence,
...
class MetricsCollector:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_error_rate': statistics.mean(self.error_rates) if self.error_rates else 0.0,
            'avg_throughput': statistics.mean(self.throughput) if self.throughput else 0.0,
            'resource_usage': {}
        }
...
class MonitoringSystem:
            ...
            try:
                ...
                for pattern, info in patterns:
                    ...
                    self.alert_manager.create_alert(
                        ...
                        metadata={'pattern': pattern.value, 'confidence': confidence, 'details': info}
                    )
        ...
        while self.monitoring_active:
            try:
                ...
                if avg_error_rate > 0.1:  # 10% error rate
                    severity = AlertSeverity.CRITICAL if avg_error_rate > 0.25 else AlertSeverity.WARNING
                    self.alert_manager.create_alert(
                        severity=severity,
    ...
    def record_failure(self, source: str, failure_type: str, severity: AlertSeverity = AlertSeverity.WARNING, 
...
if __name__ == "__main__":
    async def demo():
        ...
        for i in range(3):
            monitoring.record_failure("test_source", "connection_error", AlertSeverity.WARNING)
