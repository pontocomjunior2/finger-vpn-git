...
def analyze_stream_discrepancy():
    ...
    try:
        ...
        if server1_assignments:
            ...
            for assignment in server1_assignments[
                :10
        ...
        if heartbeats:
            ...
            for heartbeat in heartbeats:
                ...
                print(
                    f"  Server {server_id}: {timestamp} ({seconds_ago:.0f}s atrás) - Status: {status}"
                )
        else:
        ...
        try:
            ...
            if rebalance_history:
                print(
                    f"Ações de rebalanceamento nas últimas 24h: {len(rebalance_history)}"
                )
        ...
        for analysis in capacity_analysis:
            ...
            if server_id == "1":
                ...
                if current_streams == max_streams:
                    print(
                        f"  📊 EXPLICAÇÃO: Instância reporta estar na capacidade máxima"
                    )
                if active_assignments == 0:
                    print(f"  📊 REALIDADE: Não há assignments ativos no banco")
                elif active_assignments > 0:
                    print(
                        f"  📊 REALIDADE: {active_assignments} assignments ativos encontrados no banco"
                    )
