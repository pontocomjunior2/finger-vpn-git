...
from consistency_integration import (
    ConsistencyService,
...
async def demo_basic_consistency_check(db_manager):
    ...
    if report.stream_issues:
        ...
        for i, issue in enumerate(report.stream_issues[:3], 1):  # Show first 3
            print(
                f"  {i}. Stream {issue.stream_id}: {issue.issue_type.value} - {issue.description}"
            )
...
async def demo_monitoring_cycle():
    ...
    try:
        service = await setup_consistency_service(
            db_manager, {"monitoring_interval": 5}
        )
        ...
        print(
            f"📊 Monitoring completed - {len(history['recent_reports'])} checks performed"
        )
