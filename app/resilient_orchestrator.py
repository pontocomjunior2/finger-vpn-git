...
class ResilientOrchestrator:
    ...
    def _is_circuit_breaker_open(self, key: str) -> bool:
        ...
        if state == "open":
            ...
            if last_failure:
                ...
                ).total_seconds() > self.recovery_config.circuit_breaker_timeout_seconds
                if timeout_passed:
    ...
    def _record_circuit_breaker_failure(self, key: str):
        ...
        if (
            self.circuit_breaker_failures[key]
            >= self.recovery_config.circuit_breaker_threshold
        ):
            self.circuit_breaker_state[key] = "open"
            logger.warning(
                f"Circuit breaker opened for {key} after {self.circuit_breaker_failures[key]} failures"
            )
        ...
        try:
            ...
            for instance in instances:
                ...
                if heartbeat_age > self.heartbeat_config.emergency_threshold_seconds:
                    ...
                    await self._handle_emergency_instance_failure(
                        server_id, current_streams, "Emergency heartbeat timeout"
                    )

                elif heartbeat_age > self.heartbeat_config.timeout_seconds:
                    # Standard timeout - mark as failed and redistribute
                    await self._handle_instance_heartbeat_failure(
                        server_id, current_streams, "Heartbeat timeout"
                    )
                    ...
                    logger.warning(
                        f"Instance {server_id} heartbeat delayed: {heartbeat_age:.1f}s"
                    )
    ...
    async def _handle_instance_heartbeat_failure(
        self, server_id: str, current_streams: int, reason: str
    ):
    ...
    async def _handle_emergency_instance_failure(
        self, server_id: str, current_streams: int, reason: str
    ):
    ...
    async def _graceful_instance_failure_handling(self, server_id: str, reason: str):
        ...
        try:
            ...
            if not orphaned_streams:
                logger.info(
                    f"No orphaned streams found for failed instance {server_id}"
                )
                return

            logger.info(
                f"Found {len(orphaned_streams)} orphaned streams from {server_id}"
            )

            # Use enhanced orchestrator for smart redistribution if available
            if self.enhanced_orchestrator:
                try:
                    result = self.enhanced_orchestrator.handle_instance_failure(
                        server_id
                    )
                    if result.success:
                        logger.info(
                            f"Smart redistribution completed: {result.streams_moved} streams moved"
                        )
                        return
                    else:
                        logger.warning(
                            f"Smart redistribution failed: {result.error_message}"
                        )
            ...
            if self.recovery_config.emergency_recovery_enabled:
                await self._execute_emergency_recovery(
                    server_id, f"Graceful handling failed: {e}"
                )
        finally:
            cursor.close()
            conn.close()

    async def _basic_stream_redistribution(
        self, cursor, orphaned_streams: List[int], failed_server_id: str
    ):
        ...
        if assignments:
            ...
            logger.info(
                f"Redistributed {len(assignments)} streams to {len(set(a[1] for a in assignments))} instances"
            )
    ...
    async def _emergency_redistribute_streams(self, server_id: str):
        ...
        try:
            ...
            if unassigned_streams:
                await self._basic_stream_redistribution(
                    cursor, unassigned_streams, server_id
                )
    ...
    async def _emergency_verify_consistency(self, server_id: str):
        ...
        if self.enhanced_orchestrator:
            try:
                consistency_report = (
                    self.enhanced_orchestrator.verify_system_consistency()
                )
                if not consistency_report.get("consistent", False):
                    logger.warning(
                        f"System inconsistency detected after emergency recovery: {consistency_report}"
                    )
                else:
    ...
    async def _check_system_health(self):
        ...
        try:
            ...
            capacity_utilization = stats["total_streams"] / max(
                stats["total_capacity"], 1
            )
            ...
            if active_ratio < 0.5:  # Less than 50% instances active
                ...
                active_ratio < 0.8 or capacity_utilization > 0.9
            ):  # Less than 80% active or over 90% capacity
                ...
                self.critical_failures = [
                    f for f in self.critical_failures if f["failure_time"] > cutoff_time
                ]
            ...
            if self.system_health_status != previous_status:
                logger.info(
                    f"System health status changed: {previous_status.value} -> {self.system_health_status.value}"
                )
    ...
    async def _attempt_instance_recovery(self):
        ...
        for server_id, failure_info in list(self.failed_instances.items()):
            ...
            if (
                failure_info.recovery_attempts
                >= self.recovery_config.max_retry_attempts
            ):
                continue
            ...
            if failure_info.last_recovery_attempt:
                time_since_attempt = (
                    current_time - failure_info.last_recovery_attempt
                ).total_seconds()
            ...
            # Attempt recovery
            logger.info(
                f"Attempting recovery for failed instance {server_id} (attempt {failure_info.recovery_attempts + 1})"
            )
    ...
    async def _try_instance_recovery(
        self, server_id: str, failure_info: InstanceFailureInfo
    ) -> bool:
        ...
        try:
            ...
            if last_heartbeat:
                ...
                if time_since_heartbeat < self.heartbeat_config.timeout_seconds:
                    ...
                    logger.info(
                        f"Instance {server_id} showing signs of recovery (recent heartbeat)"
                    )
                    return True
    ...
    async def force_instance_recovery(self, server_id: str) -> Dict:
        ...
        if server_id in self.failed_instances:
            ...
            if success:
                ...
                return {
                    "status": "success",
                    "message": f"Instance {server_id} recovery successful",
                }
            else:
                return {
                    "status": "failed",
                    "message": f"Instance {server_id} recovery failed",
                }
        else:
            return {
                "status": "not_found",
                "message": f"Instance {server_id} not in failed state",
            }

    async def trigger_emergency_recovery(
        self, server_id: str, reason: str = "Manual trigger"
    ) -> Dict:
        ...
        try:
            ...
            return {
                "status": "success",
                "message": f"Emergency recovery completed for {server_id}",
            }
        except Exception as e:
