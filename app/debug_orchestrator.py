    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            ...
            async with session.post(
                f"{ORCHESTRATOR_URL}/register", json=register_data
            ) as response:
                if response.status == 200:
                    data = await response.json()
        ...
        try:
            ...
            async with session.post(
                f"{ORCHESTRATOR_URL}/heartbeat", json=heartbeat_data
            ) as response:
                if response.status == 200:
                    data = await response.json()
        ...
        try:
            streams_data = {"server_id": SERVER_ID, "requested_count": 5}

            async with session.post(
                f"{ORCHESTRATOR_URL}/streams/assign", json=streams_data
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    error_text = await response.text()
                    print(
                        f"❌ Solicitação de streams falhou: {response.status} - {error_text}"
                    )
        except Exception as e:
