...
class EnhancedStreamOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced orchestrator initialized with config: "
    ...
    def get_db_connection(self):
        """Get internal database connection via the manager."""
        return self.db_manager.get_connection()

    def get_streams_db_connection(self):
        """Get external streams database connection with error handling."""
        try:
            conn = psycopg2.connect(**self.postgres_config)
            conn.autocommit = True
            return conn
        except Exception as e:
            logger.error(f"External streams DB connection error: {e}")
            self.error_handler.handle_error(
                e, "Failed to connect to external streams DB"
            )
            raise
    ...
    def get_instance_metrics(self) -> List[InstanceMetrics]:
        ...
        try:
            ...
            for row in instances_data:
                ...
                failure_count = self.instance_performance_history.get(
                    row["server_id"], {}
                ).get("failure_count", 0)
    ...
    def intelligent_rebalance(
        self, reason: RebalanceReason = RebalanceReason.MANUAL
    ) -> RebalanceResult:
        """
        ...
        try:
            ...
            should_rebalance, rebalance_reason = self.load_balancer.should_rebalance(
                instances
            )
            ...
            migrations = self.load_balancer.generate_rebalance_plan(
                instances, total_streams, reason
            )
            ...
            result = self.load_balancer.execute_gradual_migration(
                migrations, self._get_server_stream_ids, self._execute_stream_migration
            )
    ...
    def _execute_stream_migration(
        self, stream_ids: List[int], from_server: str, to_server: str
    ) -> bool:
        """
    ...
    def handle_instance_failure(self, failed_server_id: str) -> RebalanceResult:
        ...
        try:
            ...
            if not orphaned_streams:
                logger.info(
                    f"No orphaned streams found for failed instance {failed_server_id}"
                )
                return RebalanceResult(
                    success=True,
            ...
            logger.info(
                f"Removed {len(orphaned_streams)} orphaned streams from {failed_server_id}"
            )

            # Redistribute orphaned streams using smart load balancer
            instances = self.get_instance_metrics()
            active_instances = [
                inst for inst in instances if inst.server_id != failed_server_id
            ]
            ...
            for server_id, target_count in optimal_distribution.items():
                ...
                streams_to_assign = orphaned_streams[
                    stream_index : stream_index + target_count
                ]
                ...
                if streams_to_assign:
                    ...
                    logger.info(
                        f"Assigned {len(streams_to_assign)} streams to {server_id}"
                    )
            ...
            self.instance_performance_history[failed_server_id]["failure_count"] = (
                self.instance_performance_history[failed_server_id].get(
                    "failure_count", 0
                )
                + 1
            )
    ...
    def verify_system_consistency(self) -> Dict:
        ...
        if not instances:
            return {"consistent": False, "error": "No active instances found"}
