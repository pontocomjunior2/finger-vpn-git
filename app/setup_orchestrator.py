...
def load_environment():
    ...
    if missing_vars:
        logger.error(
            f"Variáveis de ambiente obrigatórias não encontradas: {missing_vars}"
        )
    ...
    logger.info(
        f"Configuração do banco: {os.getenv('DB_HOST')}:{os.getenv('DB_PORT')}/{os.getenv('DB_NAME')}"
    )
    return True
...
def test_database_connection():
    ...
    try:
        ...
        logger.info(
            f"Testando conexão com banco de dados em {conn_params['host']}:{conn_params['port']}..."
        )
...
def create_database_tables():
    ...
    try:
        ...
        # Construir configurações a partir das variáveis de ambiente
        db_config = {
        ...
        # Instanciar com as configurações necessárias
        orchestrator = StreamOrchestrator(db_config=db_config, postgres_config=postgres_config)
...
def main():
    ...
    if not skip_db_test:
        if not test_database_connection():
            ...
            logger.info(
                "Para pular este teste, use: python setup_orchestrator.py --skip-db-test"
            )
    ...
    if not skip_db_test:
        ...
        logger.info(
            "Criação de tabelas pulada (será feita no primeiro boot do orquestrador)"
        )
