        ...
        if response.status_code == 200:
            ...
            for instance in instances:
                if isinstance(instance, dict):
                    print(
                        f"   - {instance.get('server_id', 'N/A')}: {instance.get('current_streams', 'N/A')}/{instance.get('max_streams', 'N/A')} streams"
                    )
        ...
        for instance in instances:
            ...
            if max_streams != 20:
                problem_instances.append(
                    f"Instância {server_id}: max_streams={max_streams} (esperado=20)"
                )
                print(f"   ⚠️  PROBLEMA: max_streams incorreto")

            if seconds_since > 300:  # 5 minutos
                problem_instances.append(
                    f"Instância {server_id}: heartbeat antigo ({int(seconds_since)}s)"
                )
...
def test_instance_registration():
    ...
    try:
        ...
        response = requests.post(
            f"{ORCHESTRATOR_URL}/register", json=test_data, timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                test_instance = next(
                    (i for i in instances if i["server_id"] == "test_debug_instance"),
                    None,
                )
                ...
                if test_instance:
                    ...
                    if test_instance["max_streams"] == 20:
                        ...
                        print(
                            f"❌ max_streams incorreto: {test_instance['max_streams']} (esperado: 20)"
                        )
                else:
