...
from monitoring_system import (AlertSeverity, MonitoringSystem,
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                health_status = {
                    ...
                    'checks': {
                        ...
                        'critical_alerts': len(self.monitoring_system.alert_manager.get_active_alerts(AlertSeverity.CRITICAL)),
                ...
                critical_alerts = self.monitoring_system.alert_manager.get_active_alerts(AlertSeverity.CRITICAL)
                if critical_alerts:
                    health_status['status'] = 'critical'
                    health_status['checks']['critical_alert_count'] = len(critical_alerts)
        ...
        async def get_alerts(
            ...
            try:
                if active_only:
                    ...
                    alerts = self.monitoring_system.alert_manager.get_alert_history(hours=hours)
        ...
        async def get_metrics(
            ...
            try:
                if metric_name:
                    metrics_data = {
                        metric_name: self.monitoring_system.metrics_collector.get_metric_history(metric_name, hours)
                    }
                else:
                    # Get all metrics
                    metrics_data = {}
                    for name in self.monitoring_system.metrics_collector.metrics.keys():
                        metrics_data[name] = self.monitoring_system.metrics_collector.get_metric_history(name, hours)
                
                # Convert to response format
                response = {}
                for name, metrics in metrics_data.items():
                    response[name] = [MetricResponse(**metric.to_dict()) for metric in metrics]
        ...
        async def get_performance():
            ...
            try:
                summary = self.monitoring_system.metrics_collector.get_performance_summary()
        ...
        async def create_test_alert(
            ...
            description: str = Query("This is a test alert", description="Alert description")
        ):
...
def create_monitoring_api(monitoring_system: Optional[MonitoringSystem] = None) -> MonitoringAPI:
    """Create monitoring API instance"""
    return MonitoringAPI(monitoring_system)


# Standalone server function
async def run_monitoring_server(host: str = "0.0.0.0", port: int = 8080, monitoring_system: Optional[MonitoringSystem] = None):
