...
from resilient_config import (get_environment_config_summary,
                              load_resilient_config)
...
class ResilientOrchestratorDemo:
    ...
    def demonstrate_configuration(self):
        ...
        logger.info(f"Warning threshold: {self.heartbeat_config.warning_threshold_seconds}s")
        logger.info(f"Emergency threshold: {self.heartbeat_config.emergency_threshold_seconds}s")
        
        # Show recovery configuration
        logger.info(f"Max retry attempts: {self.recovery_config.max_retry_attempts}")
        logger.info(f"Circuit breaker threshold: {self.recovery_config.circuit_breaker_threshold}")
        logger.info(f"Emergency recovery enabled: {self.recovery_config.emergency_recovery_enabled}")
    ...
    def demonstrate_circuit_breaker(self):
        ...
        logger.info(f"Circuit breaker status for {service_key}: {'OPEN' if self.orchestrator._is_circuit_breaker_open(service_key) else 'CLOSED'}")
        ...
        for i in range(self.recovery_config.circuit_breaker_threshold + 1):
            ...
            status = "OPEN" if self.orchestrator._is_circuit_breaker_open(service_key) else "CLOSED"
    ...
    async def demonstrate_heartbeat_monitoring(self):
        ...
        timeout_time = datetime.now() - timedelta(seconds=self.heartbeat_config.timeout_seconds + 10)
        ...
        for server_id, last_heartbeat in self.orchestrator.instance_heartbeats.items():
            ...
            status = "TIMEOUT" if age > self.heartbeat_config.timeout_seconds else "HEALTHY"
    ...
    async def demonstrate_failure_handling(self):
        ...
        logger.info(f"Simulating failure for {server_id} with {current_streams} streams...")
    ...
    async def demonstrate_system_health_monitoring(self):
        ...
        logger.info(f"Initial system health: {self.orchestrator.system_health_status.value}")
        
        # Simulate health degradation
        logger.info("Simulating system health degradation...")
        self.orchestrator.system_health_status = SystemHealthStatus.DEGRADED
        logger.info(f"System health updated to: {self.orchestrator.system_health_status.value}")
    ...
    async def demonstrate_monitoring_lifecycle(self):
        ...
        logger.info(f"Active monitoring tasks: {len(self.orchestrator.monitoring_tasks)}")
