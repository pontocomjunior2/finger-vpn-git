...
class MonitoringConfigManager:
    ...
    def get_enabled_alert_rules(self) -> Dict[str, AlertRuleConfig]:
        ...
        return {
            name: rule for name, rule in self.config.alert_rules.items()
            if rule.enabled
        }
...
def add_custom_alert_rule(name: str, metric_name: str, threshold: float, 
                         severity: str, duration: int = 300, description: str = ""):
