...
from database_error_handler import (
    DatabaseErrorHandler,
    ErrorContext,
    get_database_error_handler,
)
from enhanced_db_manager import EnhancedDatabaseManager, get_enhanced_db_manager
...
class EnhancedDatabaseSystem:
    ...
    def __init__(
        self, minconn: int = 3, maxconn: int = 15, enable_monitoring: bool = True
    ):
    ...
    def get_connection_sync(self, context: str = "unknown", timeout: float = None):
        ...
        try:
            ...
            # Handle error through error handler
            self.error_handler.handle_error_sync(e, error_context, connection_operation)
    ...
    async def execute_with_retry(
        self, operation: Callable, context: str = "unknown", max_retries: int = 3
    ) -> Any:
    ...
    def get_system_health(self, force_refresh: bool = False) -> SystemHealth:
        ...
        if avg_conn_time > self.performance_thresholds["connection_time_critical"]:
            ...
            recommendations.append(
                "Consider increasing connection pool size or optimizing database"
            )
        ...
        if error_rate > self.performance_thresholds["error_rate_critical"]:
            ...
            recommendations.append(
                "Review recent errors and optimize problematic queries"
            )
        ...
        if active_tx > self.performance_thresholds["active_transactions_critical"]:
            ...
            recommendations.append(
                "Review long-running transactions and optimize query performance"
            )
            ...
            recommendations.append(
                "Monitor transaction duration and consider optimization"
            )
        ...
        if recent_deadlocks > self.performance_thresholds["deadlock_rate_critical"]:
            ...
            recommendations.append(
                "Review transaction logic and implement deadlock prevention strategies"
            )
            ...
            recommendations.append(
                "Monitor concurrent operations and optimize transaction ordering"
            )
    ...
    def optimize_performance(self) -> Dict[str, Any]:
        ...
        if health.metrics["transactions"]["active_transactions"] > 20:
            rolled_back = self.transaction_manager.force_rollback_all_long_transactions(
                300
            )
            if rolled_back:
                optimizations.append(
                    f"Rolled back {len(rolled_back)} long-running transactions"
                )
    ...
    def emergency_recovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recovery_actions.append(
                f"Force rolled back {len(rolled_back)} transactions"
            )
            ...
            return {
                ...
                "health_after_recovery": self.get_system_health(
                    force_refresh=True
                ).score,
            }
...
def get_enhanced_database_system(
    minconn: int = 3, maxconn: int = 15
) -> EnhancedDatabaseSystem:
