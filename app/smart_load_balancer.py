...
class InstanceMetrics:
    ...
    def performance_score(self) -> float:
        ...
        # Weighted average
        score = (cpu_score * 0.3 + memory_score * 0.3 + load_score * 0.2 + 
                failure_penalty * 0.1 + response_penalty * 0.1)
...
class SmartLoadBalancer:
    ...
    def calculate_optimal_distribution(self, instances: List[InstanceMetrics], 
                                     total_streams: int) -> Dict[str, int]:
        ...
        for inst in active_instances:
            weight = (inst.max_streams * inst.performance_score) / total_weighted_capacity
    ...
    def detect_imbalance(self, instances: List[InstanceMetrics]) -> Tuple[bool, str]:
        ...
        if emergency_instances:
            return True, f"Emergency: {len(emergency_instances)} instances at >95% capacity"
        ...
        if max_load - min_load > self.config.imbalance_threshold:
            return True, f"Load imbalance: {max_load:.2f} - {min_load:.2f} > {self.config.imbalance_threshold}"
        ...
        if max_diff > self.config.max_stream_difference:
            return True, f"Stream count imbalance: max difference {max_diff:.1f} > {self.config.max_stream_difference}"
        
        return False, "System is balanced"
    
    def generate_rebalance_plan(self, instances: List[InstanceMetrics], 
                               total_streams: int, reason: RebalanceReason) -> List[MigrationPlan]:
        """Generate rebalance plan"""
        optimal_distribution = self.calculate_optimal_distribution(instances, total_streams)
        current_distribution = {inst.server_id: inst.current_streams for inst in instances}
        ...
        for inst in instances:
            ...
            if current > optimal:
                ...
                if available_capacity > 0:
                    targets.append((inst.server_id, available_capacity, inst.performance_score))
        ...
        for source_id, excess, _ in sources:
            ...
            for target_id, capacity, target_perf in targets:
                if remaining_excess <= 0 or migration_count >= self.config.max_migrations_per_cycle:
                    break
                
                migration_size = min(remaining_excess, capacity, self.config.migration_batch_size)
    ...
    def _calculate_priority(self, reason: RebalanceReason, target_performance: float) -> int:
    ...
    def should_rebalance(self, instances: List[InstanceMetrics], 
                        min_interval_seconds: int = 60) -> Tuple[bool, str]:
        """Determine if rebalancing should be performed"""
        if (self.last_rebalance and 
            datetime.now() - self.last_rebalance < timedelta(seconds=min_interval_seconds)):
            return False, f"Too soon since last rebalance ({min_interval_seconds}s minimum)"
    ...
    def get_rebalance_statistics(self) -> Dict:
        ...
        return {
            ...
            "average_execution_time_ms": sum(r.execution_time_ms for r in self.rebalance_history) / len(self.rebalance_history),
            "last_rebalance": self.last_rebalance.isoformat() if self.last_rebalance else None,
            "success_rate": len(successful) / len(self.rebalance_history) if self.rebalance_history else 0
        }