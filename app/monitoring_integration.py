...
class MonitoredOperation:
    ...
    def __enter__(self):
        self.operation_id = self.monitoring_system.record_operation_start(
            self.operation_name
        )
...
async def monitored_async_operation(
    monitoring_system: MonitoringSystem, operation_name: str
):
...
class OrchestratorMonitoringMixin:
    ...
    def record_instance_failure(
        self, server_id: str, failure_reason: str, streams_affected: int = 0
    ):
    ...
    def record_heartbeat_failure(
        self, server_id: str, last_heartbeat: datetime, timeout_seconds: int
    ):
    ...
    def record_rebalancing_event(
        self, reason: str, streams_moved: int, instances_affected: List[str]
    ):
...
class WorkerMonitoringMixin:
    ...
    def record_orchestrator_communication(
        self,
        operation: str,
        duration_ms: float,
        success: bool,
        ...
        if self.monitoring_system:
            self.monitoring_system.metrics_collector.record_response_time(
                f"orchestrator_{operation}", duration_ms
            )
    ...
    def __call__(self, func):
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                async with monitored_async_operation(
                    self.monitoring_system, self.operation_name
                ):
    ...
    async def check_orchestrator_health(self, orchestrator) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(orchestrator, "system_health_status"):
                health_status["checks"][
                    "system_health"
                ] = orchestrator.system_health_status.value
    ...
    async def check_worker_health(self, worker) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(worker, "local_state"):
                ...
                health_status["checks"]["assigned_streams"] = len(
                    worker.local_state.assigned_streams
                )
