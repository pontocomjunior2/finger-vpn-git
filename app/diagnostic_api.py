...
class DiagnosticAPI:
    ...
    def __init__(self, db_config: Optional[Dict] = None, monitoring_system: Optional[MonitoringSystem] = None):
        self.db_config = db_config or self._get_default_db_config()
        self.monitoring_system = monitoring_system or (create_monitoring_system() if HAS_MONITORING else None)
    ...
    def _setup_routes(self):
        ...
        async def fix_inconsistencies(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Fix inconsistencies failed: {str(e)}")
                ...
                inconsistencies.append(InconsistencyReport(
                    ...
                    metadata={
                        ...
                        'instances': [
                            {
                                'server_id': i[0],
                                'last_heartbeat': i[1].isoformat() if i[1] else None,
                                'status': i[2]
    ...
    async def _get_instances_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for instance in instances:
                ...
                seconds_since = instance_data.get('seconds_since_heartbeat', float('inf'))
            ...
            return {
                ...
                'summary': {
                    ...
                    ) if sum(i.get('max_streams', 0) or 0 for i in instances) > 0 else 0
                }
            }
    ...
    async def _get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            if self.monitoring_system:
                return self.monitoring_system.metrics_collector.get_performance_summary()
            else:
    ...
    async def _collect_performance_metrics(self, hours: int, requested_metrics: Optional[List[str]]) -> List[PerformanceMetrics]:
        ...
        try:
            ...
            available_metrics = list(self.monitoring_system.metrics_collector.metrics.keys())
            
            if requested_metrics:
                metrics_to_collect = [m for m in requested_metrics if m in available_metrics]
            else:
                metrics_to_collect = available_metrics
            
            for metric_name in metrics_to_collect:
                metric_history = self.monitoring_system.metrics_collector.metrics.get(metric_name, [])
    ...
    def _determine_system_status(self, components: Dict[str, Any], active_alerts: List[Dict[str, Any]]) -> str:
        ...
        for component, status in components.items():
            if isinstance(status, dict):
                ...
                elif status.get('status') == 'degraded' or status.get('health_ratio', 1.0) < 0.8:
                    unhealthy_components.append(component)
    ...
    def _generate_performance_summary(self, performance_metrics: List[PerformanceMetrics]) -> Dict[str, Any]:
        ...
        for metric in performance_metrics:
            ...
            if metric.trend == "increasing" and "error" in metric.metric_name.lower():
                critical += 1
            elif metric.trend == "increasing" and "response_time" in metric.metric_name.lower():
        ...
        return {
            ...
            'overall_performance_score': (healthy + degraded * 0.5) / len(performance_metrics) if performance_metrics else 1.0
        }
        ...
        for check in health_checks:
            if check.status == HealthStatus.CRITICAL:
                if check.component == 'database':
                    recommendations.append(f"CRITICAL: Fix database connectivity issues - {check.message}")
                elif check.component == 'instances':
                    recommendations.append(f"CRITICAL: Address instance health problems - {check.message}")
                elif check.component == 'streams':
                    recommendations.append(f"CRITICAL: Resolve stream assignment issues - {check.message}")
    ...
    async def _fix_inconsistency(self, inconsistency_id: str, dry_run: bool = False) -> Dict[str, Any]:
...
def create_diagnostic_api(db_config: Optional[Dict] = None, monitoring_system: Optional[MonitoringSystem] = None) -> DiagnosticAPI:
    """Create diagnostic API instance"""
    return DiagnosticAPI(db_config, monitoring_system)


# Standalone server function
async def run_diagnostic_server(host: str = "0.0.0.0", port: int = 8081, 
