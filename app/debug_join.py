...
def debug_join():
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM orchestrator_stream_assignments ORDER BY assigned_at DESC"
        )
        assignments = cursor.fetchall()
        print(f"\nAssignments na tabela ({len(assignments)}):")
        for assignment in assignments:
            print(
                f"  - Stream {assignment['stream_id']} -> {assignment['server_id']} ({assignment['status']})"
            )

        # 2. Verificar instances
        cursor.execute(
            "SELECT * FROM orchestrator_instances ORDER BY registered_at DESC"
        )
        instances = cursor.fetchall()
        print(f"\nInstances na tabela ({len(instances)}):")
        for instance in instances:
            print(
                f"  - {instance['server_id']}: {instance['current_streams']}/{instance['max_streams']} ({instance['status']})"
            )
        ...
        for result in join_results:
            print(
                f"  - Stream {result['stream_id']} -> {result['server_id']} @ {result['ip']}:{result['port']}"
            )

        # 5. Verificar se há assignments ativos
        cursor.execute(
            "SELECT COUNT(*) as count FROM orchestrator_stream_assignments WHERE status = 'active'"
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) as count FROM orchestrator_instances WHERE status = 'active'"
        )
