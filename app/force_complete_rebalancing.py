...
def move_stream_assignment(conn, stream_id: str, from_server: str, to_server: str):
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            print(f"⚠️ Nenhum assignment encontrado para stream {stream_id} na instância {from_server}")
            return False
...
def force_rebalance():
    ...
    print(f"🔄 REBALANCEAMENTO FORÇADO - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    ...
    try:
        ...
        available.sort(key=lambda x: x['available'], reverse=True)  # Mais capacidade primeiro
        ...
        for overloaded_instance in overloaded:
            ...
            for i in range(streams_to_move):
                ...
                # Mover o stream
                if move_stream_assignment(conn, stream_id, server_id, target_instance['server_id']):
                    print(f"  ✅ Stream {stream_id} movido: {server_id} → {target_instance['server_id']}")
