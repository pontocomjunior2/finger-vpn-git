...
class IntegratedOrchestrator:
    ...
    def __init__(self, db_config: Dict[str, str], postgres_config: Dict[str, str]):
    ...
    async def initialize_components(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"All components initialized successfully ({self.metrics.components_healthy}/{self.metrics.components_total})"
            )
            return True
    ...
    async def register_instance(
        self, server_id: str, ip: str, port: int, max_streams: int
    ) -> Dict[str, Any]:
    ...
    async def process_heartbeat(
        self, server_id: str, heartbeat_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.resilient_orchestrator:
                await self.resilient_orchestrator.process_heartbeat(
                    server_id, heartbeat_data
                )
    ...
    async def assign_streams(
        ...
        try:
            ...
            if server_id in self.active_instances and result.get("assigned_streams"):
                ...
                self.active_instances[server_id]["current_streams"] += len(
                    assigned_streams
                )
    ...
    async def handle_instance_failure(self, server_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.resilient_orchestrator:
                recovery_result = (
                    await self.resilient_orchestrator.handle_instance_failure(server_id)
                )

            # Use enhanced orchestrator for stream redistribution
            if self.enhanced_orchestrator:
                rebalance_result = (
                    await self.enhanced_orchestrator.handle_instance_failure(server_id)
                )
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if healthy_components == total_components:
                    ...
                    self.status = IntegrationStatus.FAILED
                    logger.error(
                        "System in failed state - less than 50% components healthy"
                    )
                else:
            ...
            try:
                if self.consistency_checker:
                    ...
                    if not report.is_healthy:
                        logger.warning(
                            f"Consistency issues detected: {len(report.stream_issues)} issues"
                        )

                        # Attempt auto-recovery
                        recovery_results = (
                            await self.consistency_checker.auto_recover_inconsistencies(
                                report
                            )
                        )

                        successful_recoveries = sum(
                            1 for r in recovery_results if r.success
                        )
                        logger.info(
        ...
        while not self.shutdown_event.is_set():
            try:
                if self.enhanced_orchestrator and len(self.active_instances) > 1:
                    ...
                    if self.enhanced_orchestrator.load_balancer.should_rebalance(
                        instances
                    ):
                        logger.info("Triggering automatic rebalancing")
                        await self._trigger_smart_rebalancing(
...
class EndToEndTestSuite:
        ...
        for scenario in test_scenarios:
            try:
                ...
                if result.success:
                    ...
                    logger.error(
                        f"✗ Test scenario {scenario.value} failed: {result.error_message}"
                    )
    ...
    async def _test_normal_operation(self) -> TestResult:
        ...
        try:
            ...
            for server_id, ip, port, max_streams in instances:
                result = await self.orchestrator.register_instance(
                    server_id, ip, port, max_streams
                )
                if result["status"] != "success":
                    raise Exception(
                        f"Failed to register instance {server_id}: {result.get('error')}"
                    )
            ...
            for server_id, _, _, _ in instances:
                ...
                if (
                    ...
                    and "assigned_streams" in result["result"]
                ):
            ...
            for server_id, _, _, _ in instances:
                ...
                result = await self.orchestrator.process_heartbeat(
                    server_id, heartbeat_data
                )
                if result["status"] != "success":
    ...
    async def _test_instance_failure(self) -> TestResult:
        ...
        try:
            ...
            failure_result = await self.orchestrator.handle_instance_failure(
                "test-worker-fail-1"
            )

            details["failure_simulated"] = True
            details["recovery_performed"] = failure_result.get(
    ...
    async def _test_load_imbalance(self) -> TestResult:
        ...
        try:
            ...
            for server_id, ip, port, max_streams in instances:
                ...
                if result["status"] != "success":
                    raise Exception(
                        f"Failed to register {server_id}: {result.get('error')}"
                    )
            ...
            for server_id, _, _, _ in instances:
                ...
                if result["status"] != "success":
                    raise Exception(
                        f"Failed heartbeat for {server_id}: {result.get('error')}"
                    )
            ...
            for server_id, _, _, _ in instances:
                ...
                if result["status"] != "success":
                    raise Exception(
                        f"Failed to assign streams to {server_id}: {result.get('error')}"
                    )
    ...
    async def _test_instance_failure(self) -> TestResult:
        ...
        try:
            ...
            if result["status"] != "success":
                raise Exception(
                    f"Failed to handle instance failure: {result.get('error')}"
                )
    ...
    async def _test_load_imbalance(self) -> TestResult:
        ...
        try:
            ...
            if self.orchestrator.enhanced_orchestrator:
                rebalance_result = (
                    self.orchestrator.enhanced_orchestrator.intelligent_rebalance(
                        RebalanceReason.LOAD_IMBALANCE
                    )
                )
    ...
    async def _test_consistency_issues(self) -> TestResult:
        ...
        try:
            ...
            if self.orchestrator.consistency_checker:
                report = (
                    await self.orchestrator.consistency_checker.verify_stream_assignments()
                )
                ...
                if not report.is_healthy:
                    recovery_results = await self.orchestrator.consistency_checker.auto_recover_inconsistencies(
                        report
                    )
    ...
    async def _test_performance_degradation(self) -> TestResult:
        ...
        try:
            ...
            if self.orchestrator.monitoring_system:
                active_alerts = (
                    self.orchestrator.monitoring_system.alert_manager.get_active_alerts()
                )
                details["alerts_generated"] = len(active_alerts)

                performance_summary = (
                    self.orchestrator.monitoring_system.metrics_collector.get_performance_summary()
                )
...
async def main():
    ...
    orchestrator = IntegratedOrchestrator(db_config, db_config)
    ...
    try:
        ...
        logger.info(
            f"End-to-end testing completed: {test_report['summary']['passed_tests']}/{test_report['summary']['total_tests']} tests passed"
        )
...
if __name__ == "__main__":
    ...
    if not HAS_ALL_COMPONENTS:
        logger.error(
            "Not all components are available. Please ensure all enhanced components are properly installed."
        )
        exit(1)
