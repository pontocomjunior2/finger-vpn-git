...
def load_resilient_config() -> ResilientConfig:
    ...
    try:
        config = ResilientConfig(
            ...
            heartbeat_warning_threshold_seconds=int(os.getenv("HEARTBEAT_WARNING_THRESHOLD", "120")),
            heartbeat_check_interval_seconds=int(os.getenv("HEARTBEAT_CHECK_INTERVAL", "30")),
            ...
            failure_recovery_interval_seconds=int(os.getenv("FAILURE_RECOVERY_INTERVAL", "120")),
            critical_failure_retention_hours=int(os.getenv("CRITICAL_FAILURE_RETENTION_HOURS", "24")),
            
            # Performance thresholds
            max_instance_failure_rate=float(os.getenv("MAX_INSTANCE_FAILURE_RATE", "0.1")),
            min_active_instance_ratio=float(os.getenv("MIN_ACTIVE_INSTANCE_RATIO", "0.5")),
            max_capacity_utilization=float(os.getenv("MAX_CAPACITY_UTILIZATION", "0.9"))
        )
...
def validate_resilient_config(config: ResilientConfig) -> bool:
    ...
    if config.heartbeat_warning_threshold_seconds >= config.heartbeat_timeout_seconds:
        errors.append("heartbeat_warning_threshold_seconds must be less than heartbeat_timeout_seconds")
    ...
    if config.emergency_threshold_seconds <= config.heartbeat_timeout_seconds:
        errors.append("emergency_threshold_seconds must be greater than heartbeat_timeout_seconds")
...
def get_environment_config_summary() -> Dict:
    ...
    return {
        "heartbeat": {
            ...
            "HEARTBEAT_WARNING_THRESHOLD": os.getenv("HEARTBEAT_WARNING_THRESHOLD", "120"),
        ...
        "recovery": {
            ...
            "EMERGENCY_RECOVERY_ENABLED": os.getenv("EMERGENCY_RECOVERY_ENABLED", "true")
        },
        "monitoring": {
            ...
            "CRITICAL_FAILURE_RETENTION_HOURS": os.getenv("CRITICAL_FAILURE_RETENTION_HOURS", "24")
        },
        "thresholds": {
