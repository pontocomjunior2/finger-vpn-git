...
class ConsistencyService:
    ...
    def __init__(
        ...
        self.consistency_monitor = ConsistencyMonitor(
            self.consistency_checker, check_interval=monitoring_interval
        )
    ...
    async def start(self):
        ...
        logger.info(
            f"Consistency service started with {self.monitoring_interval}s monitoring interval"
        )
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if health_status == "healthy":
                logger.info(
                    f"System health check: HEALTHY (score: {report.consistency_score:.2f})"
                )
            elif health_status == "warning":
                logger.warning(
                    f"System health check: WARNING (score: {report.consistency_score:.2f}, issues: {len(report.stream_issues)})"
                )
            else:
                logger.error(
                    f"System health check: CRITICAL (score: {report.consistency_score:.2f}, critical issues: {len(report.critical_issues)})"
                )
    ...
    async def resolve_specific_issue(self, stream_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            stream_issues = [
                issue for issue in report.stream_issues if issue.stream_id == stream_id
            ]
            ...
            # Attempt resolution
            recovery_results = await self.consistency_checker.resolve_conflicts(
                stream_issues
            )
            ...
            return {"stream_id": stream_id, "status": "error", "error": str(e)}
    ...
    async def sync_instance(self, instance_id: str) -> Dict[str, Any]:
        ...
        try:
            result = await self.consistency_checker.synchronize_instance_state(
                instance_id
            )
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_health_check": (
                self.last_health_check.timestamp.isoformat()
    ...
    def _update_stats(self, report: ConsistencyReport):
        ...
        self.stats["average_consistency_score"] = (
            current_avg * (total_checks - 1) + report.consistency_score
        ) / total_checks
...
async def setup_consistency_service(
    db_manager: EnhancedDatabaseManager, config: Dict[str, Any] = None
) -> ConsistencyService:
    """
...
async def emergency_consistency_recovery(
    db_manager: EnhancedDatabaseManager,
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            ...
            "improvement": post_recovery_report.consistency_score
